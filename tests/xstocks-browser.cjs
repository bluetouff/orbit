...
async function main(){
  ...
  try{
    ...
    await page.reload();await page.waitForSelector('[data-tile="apple-xstock"]');
    assert.equal(await page.locator('#watchCount').textContent(),'2');
    await page.locator('[data-view="xstocks"]').click();
    assert.equal(await page.locator('#filterSelect option[value="anomaly"]').evaluate(e=>e.disabled),true);
    assert.match(await page.locator('#signalList').textContent(),/exclus de la référence crypto/);
    ...
    for(const lang of ['fr','en']){
      ...
      for(const width of [1440,768,390,320]){
        ...
        await page.locator('[data-tile="apple-xstock"]').focus();await page.keyboard.press('Enter');
        ...
        assert.equal(await page.locator('#assetPane-market').isVisible(),true);
        assert.match(await page.locator('#assetContent').textContent(),lang==='fr'?/Capitalisation du token/:/Token market cap/);
    ...
    const data={snapshot:stamp,status:{coingecko_xstocks:{ok:false,last_success_at:'2020-01-01T00:00:00Z',fetched_at:stamp}},coins:[{id:'test-xstock',symbol:'testx',name:'<img src=x onerror=alert(1)>',asset_type:'xstock',current_price:1,market_cap:null,total_volume:null,has_logo:false,last_updated:'2020-01-01T00:00:00Z'}]};
