...
def snapshot(offset=0):
    ...
    return {'snapshot': stamp, 'status': {key: {'ok': True, 'fetched_at': stamp} for key in ('coingecko_markets', 'coingecko_xstocks')},
            'coins': [{'asset_type': 'xstock', 'last_updated': stamp}]}
...
class VerifyTests(unittest.TestCase):
    def test_recent_file_cannot_hide_failed_or_old_sources_or_undated_quotes(self):
        ...
        for change in ('failed', 'stale', 'unknown', 'old_quote', 'future_quote'):
            data = snapshot()
            if change == 'failed': data['status']['coingecko_xstocks']['ok'] = False
            ...
            if change == 'old_quote': data['coins'][0]['last_updated'] = snapshot(-601)['snapshot']
    ...
    def test_requires_two_advances_of_each_source_not_just_new_file_dates(self):
        ...
        with patch.object(verify.time, 'time', side_effect=lambda: clock[0]), patch.object(verify.time, 'monotonic', side_effect=lambda: clock[0]), patch.object(verify.time, 'sleep', side_effect=sleep), patch.object(verify, 'public', side_effect=response):
            with self.assertRaisesRegex(verify.CollectionError, 'did not renew twice'):
