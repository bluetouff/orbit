...
const state={tf:'24h',metric:'perf',filter:'all',lens:'perf',demo:false,universe:[],byId:new Map(),selected:new Set(),view:'top',
  global:null,macro:{dxy:104.2,us10y:4.32,proxy:true},anomCount:0,regime:{label:'NEUTRE',score:0},
  social:{},macroLive:null};
...
function toggleFav(id){
  ...
  else{ if(favFull()){showToast(`Maximum ${MAX_FAV} favoris`);return false;} state.selected.add(id); }
...
function logoSrc(c){return c&&c.id?LOGO_BASE+encodeURIComponent(c.id)+'.png':'';}
...
async function loadData(first){
  try{
    const d=await getJSON(DATA_URL+(first?'':('?t='+Date.now())));
    ...
    state.social={}; d.coins.forEach(c=>{ if(c.galaxy_score!=null)
      state.social[(c.symbol||'').toUpperCase()]={galaxy_score:c.galaxy_score,sentiment:c.sentiment,social_dominance:c.social_dominance}; });
    state.demo=false; document.getElementById('demoflag').style.display='none';
    document.getElementById('pickUniv').textContent='univers : '+d.coins.length;
  }catch(e){
    if(first){ state.demo=true; document.getElementById('demoflag').style.display='block';
      document.getElementById('updated').textContent='démo · hors-ligne';
      state.universe=DEMO; state.byId=new Map(DEMO.map(c=>[c.id,c])); state.global=DEMO_GLOBAL;
      document.getElementById('pickUniv').textContent='univers : '+DEMO.length; }
  }
}
...
function computeSignals(){
  const coins=[...state.selected].map(id=>state.byId.get(id)).filter(Boolean);
  state.anomCount=0;
  if(coins.length<4){coins.forEach(c=>{c._anom=null;c._div=null;});computeRegime(coins);return;}
  ...
  coins.forEach((c,i)=>{
    ...
    c._anom=(priceAnom||actAnom)?{price:priceAnom,vol:actAnom,pz,tz:az}:null;
    if(c._anom)state.anomCount++;
    if(az==null){c._div=null;return;}
    const d=pz-az;                                  // prix vs social/activité
...
function computeRegime(coins){
  ...
  state.regime={label:score>0.22?'RISK-ON':score<-0.22?'RISK-OFF':'NEUTRE',score,breadth};
...
const REFRESH_MS=30000;          // re-lit le snapshot statique toutes les 30 s
let _busy=false;
async function refreshData(){
  if(state.demo||_busy)return; _busy=true;
  try{
    await loadData(false);
    const t=new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});
    document.getElementById('updated').textContent='maj '+t+' · '+srcLabel();
    layout();renderStrip();
  }finally{_busy=false;}
}
...
function selectedCoins(){let list=displaySource();
  ...
  else if(state.filter==='anom')list=list.filter(c=>c._anom);return list;}
...
function layout(){
  ...
  if(!list.length){tiles.forEach(t=>t.dead=true);return;}
  let maxAbs=0;
  ...
  tiles.forEach((t,id)=>{if(!ids.has(id))t.dead=true;});
  for(const r of rects){let t=tiles.get(r.id);
    if(!t){t={x:r.x+r.w/2,y:r.y+r.h/2,w:0,h:0,coin:r.coin};tiles.set(r.id,t);}
    t.coin=r.coin;t.dead=false;t.tx=r.x;t.ty=r.y;t.tw=r.w;t.th=r.h;}
}
...
function draw(){
  ...
  for(const [id,t] of tiles){
    const tx=t.dead?t.x:t.tx,ty=t.dead?t.y:t.ty,tw=t.dead?0:t.tw,th=t.dead?0:t.th;
    t.x+=(tx-t.x)*0.22;t.y+=(ty-t.y)*0.22;t.w+=(tw-t.w)*0.22;t.h+=(th-t.h)*0.22;
    if(t.dead&&t.w<1&&t.h<1){tiles.delete(id);continue;}
    ...
    if(m>26){
      ...
      let fs=Math.min(w*0.30,hh*0.34,30);
      fs=Math.min(fs,w*0.90/Math.max(1,sym.length)/0.62);   // tient en largeur
      fs=Math.max(10,fs);
      ...
      if(img&&m>44){
        ...
        ctx.fillStyle='rgba(255,255,255,.98)';ctx.font=`700 ${fs}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;
        ctx.fillText(sym,cx2,top+ls+6);
        if(showPct){ctx.fillStyle='rgba(255,255,255,.92)';ctx.font=`600 ${pf}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;ctx.fillText(fmtPct(pct),cx2,top+ls+6+fs+3);}
      }else{
        const showPct=hh>fs*2.2+8;
        ctx.textBaseline='middle';
        ctx.fillStyle='rgba(255,255,255,.98)';ctx.font=`700 ${fs}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;
        ctx.fillText(sym,cx2,cy2-(showPct?fs*0.5:0));
        if(showPct){ctx.fillStyle='rgba(255,255,255,.92)';ctx.font=`600 ${fs*0.82}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;ctx.fillText(fmtPct(pct),cx2,cy2+fs*0.62);}
      }}
    else if(m>13){ctx.fillStyle='rgba(255,255,255,.9)';ctx.textAlign='center';ctx.textBaseline='middle';
      ctx.font=`700 ${Math.max(8,m*0.5)}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;ctx.fillText(c.symbol.toUpperCase().slice(0,4),x+w/2,y+hh/2);}
    // divergence prix/activité
    if(c._div&&m>30){ctx.font=`700 ${Math.min(m*0.3,16)}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;ctx.textAlign='right';ctx.textBaseline='top';
      ctx.fillStyle=c._div.kind==='accum'?'rgba(86,200,232,.96)':'rgba(232,176,75,.96)';
      ctx.fillText(c._div.kind==='accum'?'↗':'↘',x+w-5,y+4);}
    // anomalie (cross-sectionnelle) — anneau pulsé
    if(c._anom){const a=0.32+0.34*(Math.sin(now/380)+1)/2;
      ctx.strokeStyle=`rgba(255,255,255,${a})`;ctx.lineWidth=2;roundRect(x+1,y+1,w-2,hh-2,rad);ctx.stroke();
      if(m>26){ctx.fillStyle=`rgba(255,255,255,${0.6+a*0.4})`;ctx.font=`700 ${Math.min(m*0.26,13)}px ui-monospace,'IBM Plex Mono',Menlo,Consolas,monospace`;ctx.textAlign='left';ctx.textBaseline='top';ctx.fillText('⚡',x+5,y+3);}}
  }
...
async function openSheet(c){
  ...
  const axis=state._socialMode==='galaxy'?'social':'activité';
  const dv=c._div,an=c._anom;
  const divTxt=dv?(dv.kind==='accum'?`Accumulation — le ${axis} dépasse le prix`:`Essoufflement — le prix dépasse le ${axis}`):'Alignée — pas de divergence notable';
  const divCol=dv?(dv.kind==='accum'?'hsl(193,72%,64%)':'var(--accent)'):'var(--ink-dim)';
  const divArrow=dv?(dv.kind==='accum'?'↗':'↘'):'≈';
  const anTxt=an?[an.price?'prix >2σ':null,an.vol?(state._socialMode==='galaxy'?'social >2σ':'volume >2σ'):null].filter(Boolean).join(' · '):'rien d\'inhabituel vs pairs';
  const socTop=hasSoc?'Social (LunarCrush)':'Activité (proxy social)';
  const socSrc=hasSoc?'galaxy score':'→ LunarCrush en prod';
  const socBody=hasSoc
    ?`Galaxy Score ${Number(soc.galaxy_score)||0}/100 · sentiment ${soc.sentiment!=null?(Number(soc.sentiment)||0)+'%':'—'}${soc.social_dominance!=null?' · dominance '+(+soc.social_dominance).toFixed(1)+'%':''}.`
    :'Indice dérivé du turnover (volume/cap). Galaxy Score™, sentiment et mentions se branchent sur LunarCrush via un proxy serveur.';
  ...
  <div class="grid4">
    ...
    <div class="cell"><div class="k">Offre circ.</div><div class="v">${c.circulating_supply?(c.circulating_supply/1e6).toFixed(1)+'M':'—'}</div></div>
  </div>
  ...
  <div class="sig"><div class="top"><span>Divergence prix / ${axis}</span><span class="zsc">z prix ${(c._pz||0).toFixed(1)} · z ${axis} ${(c._tz||0).toFixed(1)}</span></div>
    <div class="sigline" id="divLine"><b>${divArrow}</b> ${divTxt}</div></div>
  <div class="sig"><div class="top"><span>Scanner d'anomalies</span><span class="zsc">cross-sectionnel</span></div>
...
function openPicker(){picker.classList.add('show');pickInput.value='';state._page=0;renderPick('');}
...
function renderPick(q){
  ...
  document.getElementById('pickCount').textContent=state.selected.size+' sélectionnées';
  if(!list.length){
    pickList.innerHTML=`<div class="pick-empty">${favOnly?'aucun favori sélectionné — décoche « favoris » pour parcourir le top '+state.universe.length:'rien trouvé'}</div>`;
    return;
  }
  const rangeNote=paged?`<div class="pick-note">rang ${state._page*PAGE_SIZE+1}–${state._page*PAGE_SIZE+list.length} sur ${base.length}</div>`:'';
  ...
  pickList.querySelectorAll('.row').forEach(r=>r.addEventListener('click',()=>{const id=r.dataset.id;
    ...
    if(!ok)return;                                   // cap atteint : aucun changement
...
document.getElementById('searchBtn').addEventListener('click',()=>{search.classList.add('show');searchInput.value='';renderSearch('');setTimeout(()=>searchInput.focus(),200);});
...
function renderSearch(q){q=q.trim().toLowerCase();
  ...
  searchList.innerHTML=list.map(c=>{const pct=c[TF[state.tf]];
    return `<div class="row" data-id="${esc(c.id)}"><img src="${esc(logoSrc(c))}">
      ...
      <div class="pr">${fmtPrice(c.current_price)}<small class="${perfClass(pct)}">${fmtPct(pct)}</small></div></div>`;}).join('')
    ||`<div class="pick-empty">rien dans ta liste</div>`;
...
function renderStrip(){
  ...
  const avg=sel.length?sel.reduce((s,c)=>s+(c[TF[state.tf]]||0),0)/sel.length:0;
  document.getElementById('subline').innerHTML=
    `<span><b>${state.selected.size}</b> suivies</span>`+
    `<span>moy ${state.tf.toUpperCase()} <b class="${avg>=0?'up':'dn'}">${fmtPct(avg)}</b></span>`+
    `<span class="anom ${state.filter==='anom'?'on':''}" id="anomChip">⚡ ${state.anomCount} ${state.anomCount>1?'anomalies':'anomalie'}</span>`;
...
async function boot(){
  ...
  state.view='top';                       // default page shows the top 100
  updateFavUI();
  document.getElementById('viewSeg').querySelectorAll('button').forEach(b=>b.classList.toggle('on',b.dataset.v==='top'));
  document.getElementById('status').style.display='none';
  layout();renderStrip();movePill();draw();
  if(!state.demo){const t=new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});document.getElementById('updated').textContent='maj '+t+' · '+srcLabel();}
}
